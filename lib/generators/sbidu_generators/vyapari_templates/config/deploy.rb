...
set :repo_url,        'git@github.com:right-solutions/<%= app_name %>.git'
...
namespace :deploy do
  ...
  task :create_database do
    on primary fetch(:migration_role) do
      ...
      execute "mysql --user=$MYSQL_USERNAME --password=$MYSQL_PASSWORD -e \"CREATE DATABASE IF NOT EXISTS <%= app_name %>_production\""
      execute "mysql --user=$MYSQL_USERNAME --password=$MYSQL_PASSWORD -e \"GRANT ALL PRIVILEGES ON <%= app_name %>_production.* TO '$MYSQL_USERNAME'@'localhost' IDENTIFIED BY '$MYSQL_USERNAME' WITH GRANT OPTION\""
    end
  end
